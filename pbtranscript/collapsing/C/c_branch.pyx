__ALL__ = ["exon_finding",  "intervals_all_adjacent", "exon_matching"]

cimport numpy as np
import numpy as np
from cpython cimport bool
from pbtranscript.collapsing.intersection_unique import IntervalTreeUnique, Interval
...
def exon_finding(np.ndarray[DTYPE_t, ndim=1] baseC, 
                 np.ndarray[DTYPE_t, ndim=1] altC_neg,
                 np.ndarray[DTYPE_t, ndim=1] altC_pos, 
                 int size, int threshSplit, int threshBase, int offset):
    cdef int i
    cdef bool tag = False
    cdef index = 0
...
def exon_matching(exon_tree, ref_exon, 
                  match_extend_tolerate_left, match_extend_tolerate_right, 
